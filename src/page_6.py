...
def render_page():
    ...
    for alarm in filtered_alarms:
        ...
        # 预先构建按钮HTML，避免在f-string中嵌套表达式
        if alarm['status'] == '未处理':
            button_html = f'<button onclick="alert(\'处理告警: {alarm["id"]}\')" style="background: {COLORS["primary"]}; color: white; border: none; border-radius: 4px; padding: 4px 12px; font-size: 12px; cursor: pointer;">处理</button>'
        else:
            button_html = '<span style="color: #00B42A; font-size: 12px;">✓</span>'
        
        # ✅ 使用format或预先计算的变量，避免复杂嵌套
        alarm_html = f"""
        <div style="background: {COLORS['secondary']}; border-radius: 12px; padding: 16px; margin-bottom: 12px;
            ...
            <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                ...
                <div style="display: flex; align-items: center; gap: 8px; min-width: 80px; justify-content: flex-end;">
                    <span style="color: {status_color}; font-size: 12px; font-weight: 500;">{alarm['status']}</span>
                    {button_html}
                </div>
            </div>
        </div>
