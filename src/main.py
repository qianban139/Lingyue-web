...
def get_base64_image(image_path):
    ...
    try:
        ...
        st.warning(f"警告：未找到默认 Logo 文件 {image_path}，将显示默认图标。")
        return None
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        image_format = "png" # 假设默认是 png
        image_html = f'<img src="data:image/{image_format};base64,{logo_base64}" style="width: 90px; height: 90px; border-radius: 10px;">'
        
        st.markdown(f"""
        <div style="padding: 20px 0; text-align: center; border-bottom: 1px solid rgba(100, 116, 139, 0.2); margin-bottom: 20px;">
            <div style="display: flex; align-items: center; justify-content: center; gap: 12px;">
            <div style="width: 90px; height: 90px; border-radius: 10px; display: flex; align-items: center; justify-content: center;">
                {image_html}
            </div>
