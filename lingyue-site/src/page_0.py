...
def render_page():
    ...
    videos_config = {
        "feature": {
            ...
            "file": "assets/videos/feature.mp4"  # 改为相对仓库根目录的路径
        },
        "innovation": {
            "title": "创新亮点",
    ...
    def get_video_bytes(video_key):
        """获取视频字节流"""
        video_rel_path = videos_config[video_key]["file"]
        video_path = get_asset_path(video_rel_path)
        
        if video_path.exists():
            return open(video_path, "rb"), str(video_path)
    ...
    with main_cols[0]:
        ...
        # 视频播放器区域
        video_file, video_path_used = get_video_bytes(current_key)
        ...
        if video_file:
            ...
            <div style="background: rgba(15, 23, 42, 0.8); border-radius: 12px; height: 400px; display: flex; flex-direction: column; align-items: center; justify-content: center; border: 2px dashed rgba(100, 116, 139, 0.3);">
                ...
                <p style="color: #475569; font-size: 13px; margin-top: 8px; word-break: break-all; padding: 0 20px; text-align: center;">查找路径: {video_path_used}</p>
            </div>
            """, unsafe_allow_html=True)
    ...
    for idx, (col, img_conf) in enumerate(zip(img_cols, images_config)):
        with col:
            img_path = get_asset_path(img_conf["path"])
            
            if img_path.exists():
                st.image(
                    str(img_path), 
                    use_container_width=True,  # 已修复弃用警告
                    caption=""
                )
                st.markdown(f"""
                ...
                <div style="background: rgba(15, 23, 42, 0.6); border-radius: 12px; height: 280px; display: flex; flex-direction: column; align-items: center; justify-content: center; border: 2px dashed rgba(100, 116, 139, 0.2); margin-bottom: 10px;">
                    ...
                    <div style="color: #64748b; font-size: 11px; margin-top: 4px; word-break: break-all; padding: 0 10px; text-align: center;">{img_path}</div>
                </div>
                """, unsafe_allow_html=True)
        ...
        with col:
            ...
            if img_path.exists():
                ...
                <div style="background: rgba(15, 23, 42, 0.6); border-radius: 8px; height: 200px; display: flex; flex-direction: column; align-items: center; justify-content: center; border: 2px dashed rgba(239, 68, 68, 0.3); margin-bottom: 10px;">
                    ...
                    <div style="color: #ef4444; font-size: 11px; text-align: center; padding: 0 5px;">{img_conf['title']}</div>
                </div>
                """, unsafe_allow_html=True)
    ...
    for row_idx in range(2):
        ...
        for idx, img_conf in enumerate(snow_equipment_images[start_idx:end_idx]):
            with cols[idx]:
                ...
                # 根据类型设置颜色主题
                color_map = {
                    "snow": ("#06b6d4", "rgba(6, 182, 212, 0.1)", "rgba(6, 182, 212, 0.2)"),
                    "equipment": ("#3b82f6", "rgba(59, 130, 246, 0.1)", "rgba(59, 130, 246, 0.2)"),
                    "mixed": ("#8b5cf6", "rgba(139, 92, 246, 0.1)", "rgba(139, 92, 246, 0.2)"),
                    "defect": ("#ef4444", "rgba(239, 68, 68, 0.1)", "rgba(239, 68, 68, 0.2)")
                }
                ...
                if img_path.exists():
                    ...
                    <div style="background: rgba(15, 23, 42, 0.6); border-radius: 8px; height: 200px; display: flex; flex-direction: column; align-items: center; justify-content: center; border: 2px dashed {text_color}; margin-bottom: 10px;">
                        ...
                        <div style="color: {text_color}; font-size: 11px; text-align: center; padding: 0 5px;">{img_conf['title']}</div>
                    </div>
                    """, unsafe_allow_html=True)
