...
def get_image_html(image_path, fallback_text, height=120):
    ...
    try:
        ...
        # 尝试多种可能的路径（适配本地开发和GitHub部署）
        possible_paths = [
            current_dir / image_path,                    # 相对脚本路径
            current_dir / ".." / image_path,             # 上级目录
            current_dir / "assets" / "images" / Path(image_path).name,  # assets/images/文件名
            current_dir / ".." / "assets" / "images" / Path(image_path).name,
        ...
        if full_path and full_path.exists():
            ...
            # 调试信息：显示查找过的路径
            debug_info = f"({image_path})"
            return f'<div style="height: {height}px; background: rgba(100,116,139,0.2); border-radius: 6px; display: flex; align-items: center; justify-content: center; color: #64748b; font-size: 12px; flex-direction: column;"><span>{fallback_text}</span><span style="font-size: 10px; margin-top: 4px;">{debug_info}</span></div>'
    except Exception as e:
        return f'<div style="height: {height}px; background: rgba(100,116,139,0.2); border-radius: 6px; display: flex; align-items: center; justify-content: center; color: #64748b; font-size: 12px;">{fallback_text}<br><span style="font-size: 10px;">{str(e)}</span></div>'
...
def render_page():
    ...
    with col2:
        ...
        with filter_cols[2]:
            # 修复：使用 key 参数避免重复按钮警告
            if st.button('🔄 刷新数据', use_container_width=True, key='refresh_btn_page1'):
                with st.spinner('刷新中...'):
                    refresh_data()
                st.rerun()
