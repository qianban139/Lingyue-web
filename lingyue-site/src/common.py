...
def get_asset_path(relative_path):
    """获取资源文件的绝对路径（适配GitHub部署）"""
    # 获取当前文件所在目录
    current_dir = Path(__file__).parent.resolve()
    
    # 尝试多种路径组合
    possible_paths = [
        current_dir / relative_path,                    # 相对当前文件
        current_dir / ".." / relative_path,             # 上级目录
        current_dir / "assets" / Path(relative_path).name,  # assets/文件名
        current_dir / ".." / "assets" / Path(relative_path).name,
