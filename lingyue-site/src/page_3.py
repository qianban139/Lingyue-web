...
def create_temperature_chart():
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            ...
            gridcolor='rgba(255,255,255,0.1)', 
            tickfont=dict(color=COLORS['gray_400']),
            # ✅ 修复：改为 title=dict(text='...', font=dict(...))
            title=dict(
                text='温度 (℃)', 
                font=dict(color=COLORS['gray_400'])
            )
...
def create_risk_heatmap():
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        showscale=True,
        # ✅ 修复：colorbar的titlefont也要改为title.font
        colorbar=dict(
            title=dict(
                text='风险等级', 
                font=dict(color=COLORS['light'])
...
def render_page():
    ...
    with ai_cols[0]:
        ...
        # ✅ 分离图片显示：使用 st.image 确保正常渲染
        img_path_1 = "../assets/images/t_1.png"
        img_path_1 = Path(img_path_1)
        if img_path_1.exists():
            st.image(str(img_path_1), use_container_width=True)
        else:
            st.markdown('<div style="height: 200px; background: rgba(100,116,139,0.1); border-radius: 8px; display: flex; align-items: center; justify-content: center; color: #64748b;">传统方法示例图</div>', unsafe_allow_html=True)
    ...
    with ai_cols[1]:
        ...
        # ✅ 分离图片显示
        img_path_2 = "../assets/images/26.png"
        img_path_2 = Path(img_path_2)
        if img_path_2.exists():
            st.image(str(img_path_2), use_container_width=True)
        else:
            st.markdown('<div style="height: 200px; background: rgba(22,93,255,0.1); border-radius: 8px; display: flex; align-items: center; justify-content: center; color: #60a5fa;">AI识别示例图</div>', unsafe_allow_html=True)
